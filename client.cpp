...
int main()
{	
	int client_socket, bind_status, connect_stat;
	...
	connect_stat = connect(client_socket, (sockaddr*) &s_socket_adr, sin_size);
	if (connect_stat == -1)
	{
	...
	std::cout<<"Enter string to send : "<<std::endl; scanf("%s", send_buffer);
	if(send(client_socket, send_buffer, strlen(send_buffer), 0) == -1)
	{
	...
	{
		std::cout<<"Recieved from server : "<<recv_buffer<<std::endl;
	}
